...
class TransactionTest(TestCase):
    def test_transaction(self):
        sender = Client(name="TestSender", balance=125)
        sender.save()

        receiver = Client(name="TestReceiver", balance=95)
        receiver.save()

        transaction = Transaction(sender=sender, receiver=receiver,sum=100)
        transaction.save()
        assert transaction.sender==sender
        assert transaction.receiver_name==receiver
